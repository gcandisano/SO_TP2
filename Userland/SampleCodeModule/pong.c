...
static char shouldDraw(int x, int y, int radius, int i, int j) {
	return i * i + j * j <= radius * radius &&  // Only draw circle of the square
	       ...
	       !(x + i >= player2.x && x + i <= player2.x + PLAYER_WIDTH - 1 && y + j >= player2.y &&
	         y + j <= player2.y + PLAYER_HEIGHT);
}
	...
	for (int i = 0; i < 2; i++) {
		if (moves[i] == PLAYER1_UP_PRESSED) {
			if (player1.y > P_MIN_H) {
				drawPaddle(player1.x,
				           player1.y + PLAYER_HEIGHT - PLAYER1_MOVE_AMOUNT,
				           PLAYER_WIDTH,
				           PLAYER1_MOVE_AMOUNT,
				           BLACK);
			...
			if (player2.y > P_MIN_H) {
				drawPaddle(player2.x,
				           player2.y + PLAYER_HEIGHT - PLAYER2_MOVE_AMOUNT,
				           PLAYER_WIDTH,
				           PLAYER2_MOVE_AMOUNT,
				           BLACK);
	...
	while (player1.score < 3 && player2.score < 3) {
		...
		while (!goalMade) {
			...
			if (tick()) {
				...
				if (ballX - ballRadius / 2 <= player1.x + 2 * PLAYER_WIDTH && ballX >= player1.x &&
				    ballY >= player1.y && ballY <= player1.y + PLAYER_HEIGHT) {
					ballXDirection = 1;
					paddleHitSound();
				} else if (ballX + ballRadius / 2 + PLAYER_WIDTH >= player2.x && ballX <= player2.x + PLAYER_WIDTH &&
				           ballY >= player2.y && ballY <= player2.y + PLAYER_HEIGHT) {
					ballXDirection = -1;
					paddleHitSound();
				}
			}
